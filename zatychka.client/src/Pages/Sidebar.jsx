...
export default function Sidebar() {
    ...
    const computeInitialSection = () => {
        ...
        if (['workspace', 'transactions', 'disputes', 'payment', 'devices', 'notifications'].includes(first)) return 'priem';
        return 'main';
    };
    ...
    useEffect(() => {
        ...
        const updateTime = () => {
            ...
            (async () => {
                try {
                    const { rate: r } = await fetchUsdtRub();
                    if (!alive) return;
                    const numeric = typeof r === 'number' ? r : Number(String(r).replace(',', '.'));
                    if (!Number.isFinite(numeric)) throw new Error(`Bad rate from API: ${r}`);
                    setRate(numeric);
    ...
    function toDisplay(amountUsdt, currency) {
        const n = currency === 'USDT' ? amountUsdt : amountUsdt * (Number(rate) || 0);
    ...
    function openEdit(which) {
        ...
        setErr('');
        if (which === 'main') {
            setDraft(d => ({ ...d, main: String(curMain === 'USDT' ? mainUsdt : mainUsdt * (Number(rate) || 0)) }));
            setEdit(e => ({ ...e, main: true }));
        } else {
            setDraft(d => ({ ...d, frozen: String(curFrozen === 'USDT' ? frozenUsdt : frozenUsdt * (Number(rate) || 0)) }));
    ...
    async function commitEdit(which) {
        ...
        try {
            ...
            if (which === 'main') {
                const value = Number(String(draft.main ?? '').replace(',', '.').trim());
                if (!Number.isFinite(value)) throw new Error('Некорректное число');
                const newMainUsdt = curMain === 'USDT' ? value : value / (Number(rate) || 1);
                const saved = await SAVE({ mainUsdt: newMainUsdt, frozenUsdt });
                setMainUsdt(Number(saved?.MainUsdt ?? saved?.mainUsdt ?? newMainUsdt));
                setEdit(e => ({ ...e, main: false }));
            } else {
                const value = Number(String(draft.frozen ?? '').replace(',', '.').trim());
                if (!Number.isFinite(value)) throw new Error('Некорректное число');
                const newFrozenUsdt = curFrozen === 'USDT' ? value : value / (Number(rate) || 1);
                const saved = await SAVE({ mainUsdt, frozenUsdt: newFrozenUsdt });
                setFrozenUsdt(Number(saved?.FrozenUsdt ?? saved?.frozenUsdt ?? newFrozenUsdt));
    ...
    return (
        <>
            ...
            <div className="mobile-header" role="banner">
                <div className="mobile-header__left" onClick={() => navigate('/statistics')}>
                    <img src={logo} alt="SHARQ" className="mobile-logo" />
                </div>
            ...
            <div
                ...
                style={{ '--mobile-header-height': `${headerHeight}px` }}
            >
                <div className="sidebar-header">
                    <img src={logo} alt="SHARQ" className="logo" />
                    ...
                    {isAdmin && editMode && (
                        <div className="mode-switch">
                            <label>
                                <input
                                    type="radio" name="sb_mode" value="public"
                            ...
                            <label>
                                <input
                                    type="radio" name="sb_mode" value="private"
                    ...
                    <div className="sidebar-balance">
                        <div className="balance-box">
                            ...
                            {!edit.main ? (
                                <div
                                    className={`balance-amount ${isAdmin && editMode ? 'clickable' : ''}`}
                                    title={isAdmin && editMode ? 'Нажмите, чтобы изменить' : ''}
                                    onClick={() => (isAdmin && editMode) && openEdit('main')}
                                >
                            ...
                            <select
                                ...
                                disabled={edit.main || (isAdmin && editMode)}
                            >
                                <option>USDT</option>
                                <option>RUB</option>
                            </select>
                        ...
                        <div className="balance-box">
                            ...
                            {!edit.frozen ? (
                                <div
                                    className={`balance-amount ${isAdmin && editMode ? 'clickable' : ''}`}
                                    title={isAdmin && editMode ? 'Нажмите, чтобы изменить' : ''}
                                    onClick={() => (isAdmin && editMode) && openEdit('frozen')}
                                >
                            ...
                            <select
                                ...
                                disabled={edit.frozen || (isAdmin && editMode)}
                            >
                                <option>USDT</option>
                                <option>RUB</option>
                            </select>
                ...
                <div className="sidebar-section">
                    ...
                    {openSection === 'main' && (
                        <ul className="menu-list" onClick={closeMobileMenu}>
                            <li><NavLink to="/statistics" className={({ isActive }) => isActive ? 'active' : ''}>Статистика</NavLink></li>
                            <li><NavLink to="/balance" className={({ isActive }) => isActive ? 'active' : ''}>Баланс</NavLink></li>
                            <li><NavLink to="/referrals" className={({ isActive }) => isActive ? 'active' : ''}>Рефералы</NavLink></li>
                        </ul>
                    )}
                </div>
                ...
                <div className="sidebar-section">
                    ...
                    {openSection === 'priem' && (
                        <ul className="menu-list" onClick={closeMobileMenu}>
                            <li><NavLink to="/payin/workspace" className={({ isActive }) => isActive ? 'active' : ''}>Рабочая зона</NavLink></li>
                            <li><NavLink to="/payin/transactions" className={({ isActive }) => isActive ? 'active' : ''}>Транзакции</NavLink></li>
                ...
                <div className="sidebar-section">
                    ...
                    {openSection === 'quasi' && (
                        <ul className="menu-list" onClick={closeMobileMenu}>
                            <li><NavLink to="/quasi/payment" className={({ isActive }) => isActive ? 'active' : ''}>Транзакции</NavLink></li>
                            <li><NavLink to="/quasi/disuptes" className={({ isActive }) => isActive ? 'active' : ''}>Споры</NavLink></li>
                        </ul>
                    )}
                </div>
                ...
                <div className="sidebar-footer">
                    <div className="footer-scroll-wrapper" ref={scrollRef}>
                        <div className="footer-links">
                            <button className="footer-btn"
                                onClick={() => window.open('https://t.me/ebrag_wd', '_blank')}                        >
                    ...
                    <div className="footer-user">
                        <div className="user-email">
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><g fill="none" fillRule="evenodd"><path fill="currentColor" d="M12 2c5.523 0 10 4.477 10 10a9.96 9.96 0 0 1-2.258 6.33l.02.022l-.132.112A9.98 9.98 0 0 1 12 22c-2.95 0-5.6-1.277-7.43-3.307l-.2-.23l-.132-.11l.02-.024A9.96 9.96 0 0 1 2 12C2 6.477 6.477 2 12 2m0 15c-1.86 0-3.541.592-4.793 1.406A7.97 7.97 0 0 0 12 20a7.97 7.97 0 0 0 4.793-1.594A8.9 8.9 0 0 0 12 17m0-13a8 8 0 0 0-6.258 12.984C7.363 15.821 9.575 15 12 15s4.637.821 6.258 1.984A8 8 0 0 0 12 4m0 2a4 4 0 1 1 0 8a4 4 0 0 1 0-8m0 2a2 2 0 1 0 0 4a2 2 0 0 0 0-4" /></g></svg>
                            <span><strong>{user?.email ?? 'Гость'}</strong></span>
                        </div>
                        <>
                        <button className="logout-btn" onClick={() => setShowLogoutModal(true)}>
                            Выйти
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="m17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 0 1-3 3H6a3 3 0 0 1-3-3V7a3 3 0 0 1 3-3h4a3 3 0 0 1 3 3v1" /></svg>
                        </button>
                            {showLogoutModal && (
                                <LogOutModal
                                    email={user?.email ?? ''}               
