...
import { lookupDevices, lookupRequisites } from '../api/payin';
import { backfillByMonth } from '../api/payin';
...
export default function TransactionsBackfillModal({ open, onClose, onDone }) {
    ...
    async function searchDevices() {
        try {
            const list = await lookupDevices(devLogin || '', 50);
            setDevList(list);
        } catch (e) {
    ...
    async function searchRequisites() {
        try {
            const list = await lookupRequisites(rqLogin || '', 50);
            setRqList(list);
        } catch (e) {
    ...
    function addPair() {
        ...
        const dLabel = devList.find(x => x.id === dId)?.name || `Device #${dId}`;
    ...
    async function run() {
        try {
            ...
            const res = await backfillByMonth(payload);
        ...
        <div className="tbm-overlay" onClick={onClose}>
            <div className="tbm-modal" onClick={(e) => e.stopPropagation()}>
                <div className="tbm-head">
                    <h3>Генерация за месяц (без связок)</h3>
                ...
                <div className="tbm-row group">
                    <div className="tbm-col">
                        <label>Поиск устройств (по логину)</label>
                    ...
                    <div className="tbm-col">
                        <label>Поиск реквизитов (по логину)</label>
                        <div className="inline">
                        ...
                        <table className="tbm-table">
                            <thead>
                                <tr>
                                    <th>Устройство</th>
                                    <th>Реквизит</th>
                                    <th>Мин. сумма (USDT)</th>
                                    <th>Макс. сумма (USDT)</th>
                                    <th>Лимит в день</th>
                                    <th>Лимит в месяц</th>
                                    <th></th>
                                </tr>
                            ...
                            <tbody>
                                {pairs.map(p => (
                                    <tr key={p.id}>
                                        ...
                                        <td>
                                            <input
                                                className="tbm-input table-input"
                                                type="number"
                                                min="0"
                                                step="0.01"
                                                value={p.min}
                                                onChange={e => updatePair(p.id, 'min', e.target.value)}
                                            />
                                        </td>
                                        <td>
                                            <input
                                                ...
                                                step="0.01"
                                                value={p.max}
                                                onChange={e => updatePair(p.id, 'max', e.target.value)}
                                            />
                                        </td>
                                        <td>
                                            <input
                                                ...
                                                step="1"
                                                value={p.daily}
                                                onChange={e => updatePair(p.id, 'daily', e.target.value)}
                                            />
                                        </td>
                                        <td>
                                            <input
                                                ...
                                                step="1"
                                                value={p.monthly}
                                                onChange={e => updatePair(p.id, 'monthly', e.target.value)}
                                            />
                                        </td>
                                        <td>
                                            <button className="tbm-remove" onClick={() => removePair(p.id)}>Удалить</button>
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
